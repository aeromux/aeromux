...
- **Web Map Icon Rotation for Heading-Only Aircraft** — Fixed aircraft icons always pointing north when velocity comes from TC 19 subtype 3-4 (airspeed-based). These aircraft report heading instead of track because they lack GNSS, and typically also depend on MLAT for position. The map icon rotation now falls back to heading when track is unavailable.
