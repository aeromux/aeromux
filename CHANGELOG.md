...
- **Web Map: Traffic-Density Heatmap** — A new optional map overlay colours the coverage area by how many *distinct* aircraft passed through each grid square over a rolling window (default 24 hours), from green (quiet) to red (busiest), using a logarithmic scale anchored to a smoothed 99th-percentile so busy and quiet squares stay comparable. Squares are a fixed size in nautical miles regardless of the chosen distance unit, and appear only where aircraft were actually seen. The overlay is off by default and toggled from the settings panel, where the square size (2–40 nm) and window (1–24 h) are adjustable live; hovering a square shows its exact aircraft count, and a colour-scale legend appears in the settings panel. Server-side collection runs in memory (no persistence) and can be disabled with the new `heatmap.collect` config option to save memory on constrained hosts, which greys out the toggle.
