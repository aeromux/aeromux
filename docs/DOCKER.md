...
| Tag       | Description                                             |
|-----------|---------------------------------------------------------|
| `0.7.6`   | Version-specific, multi-arch manifest (arm64 + amd64)   |
| `latest`  | Points to the most recent version                       |

Multi-arch manifests are used so that a single tag (e.g., `0.7.6`) automatically resolves to the correct architecture when pulled.
...
```
artifacts/docker/
  aeromux_0.7.6_linux_arm64.tar
  aeromux_0.7.6_linux_amd64.tar
```

Load a saved image with:

```bash
docker load -i artifacts/docker/aeromux_0.7.6_linux_arm64.tar
```
