...
| Tag       | Description                                             |
|-----------|---------------------------------------------------------|
| `0.6.2`   | Version-specific, multi-arch manifest (arm64 + amd64)   |
| `latest`  | Points to the most recent version                       |

Multi-arch manifests are used so that a single tag (e.g., `0.6.2`) automatically resolves to the correct architecture when pulled.
...
```
artifacts/docker/
  aeromux_0.6.2_linux_arm64.tar
  aeromux_0.6.2_linux_amd64.tar
```

Load a saved image with:

```bash
docker load -i artifacts/docker/aeromux_0.6.2_linux_arm64.tar
```
